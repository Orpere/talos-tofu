...
kubectl --kubeconfig="./kubeconfig" get nodes --no-headers | grep -v control-plane | awk '{print $1}' | xargs -I{} kubectl --kubeconfig="kubeconfig" label node {} node-role.kubernetes.io/worker= --overwrite 
